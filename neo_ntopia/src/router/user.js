...
router.post('/:id(\\d+)/edit', auth.requireAuth, (req, res) => {
  const r = userService.updateProfile(req.params.id, {
...
router.post('/:id(\\d+)/avatar', auth.requireAuthAPI, async (req, res) => {
