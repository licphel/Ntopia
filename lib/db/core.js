...
function runCleanup() {
  ...
  db.exec(`
    DELETE FROM comments WHERE post_id IN (SELECT id FROM posts WHERE is_deleted = 1 AND deleted_at < datetime('now', '-${RET}'));
    DELETE FROM comments WHERE is_deleted = 1 AND deleted_at < datetime('now', '-${RET}');
    DELETE FROM posts WHERE is_deleted = 1 AND deleted_at < datetime('now', '-${RET}');
    DELETE FROM email_codes WHERE expires_at < datetime('now');
    DELETE FROM likes WHERE user_id IN (SELECT id FROM users WHERE deleted_at IS NOT NULL AND deleted_at < datetime('now', '-${RET}'));
    DELETE FROM bookmarks WHERE user_id IN (SELECT id FROM users WHERE deleted_at IS NOT NULL AND deleted_at < datetime('now', '-${RET}'));
    DELETE FROM users WHERE deleted_at IS NOT NULL AND deleted_at < datetime('now', '-${RET}');
    DELETE FROM messages WHERE is_deleted = 1 AND deleted_at < datetime('now', '-${RET}');
  `);
}
...
function init(opts) {
  ...
  db.exec(`
    ...
    -- Full-text search
    CREATE VIRTUAL TABLE IF NOT EXISTS posts_fts USING fts5(title, content_md, tags, content=posts, content_rowid=id, tokenize='unicode61 remove_diacritics 2');
  ...
  // Populate FTS index for existing posts (idempotent)
  db.exec(`INSERT INTO posts_fts(rowid, title, content_md, tags)
    SELECT id, title, content_md, tags FROM posts WHERE id NOT IN (SELECT rowid FROM posts_fts)`);

  // FTS triggers — keep index in sync with posts
  db.exec(`
    CREATE TRIGGER IF NOT EXISTS posts_fts_insert AFTER INSERT ON posts BEGIN
      INSERT INTO posts_fts(rowid, title, content_md, tags) VALUES (new.id, new.title, new.content_md, new.tags);
    END;
    CREATE TRIGGER IF NOT EXISTS posts_fts_delete AFTER DELETE ON posts BEGIN
      INSERT INTO posts_fts(posts_fts, rowid, title, content_md, tags) VALUES('delete', old.id, old.title, old.content_md, old.tags);
    END;
...
// Periodic cleanup — called at startup and on interval

module.exports = { db, init, runCleanup };
