...
// SQLite datetime relative to now: sqlFromNow('+1 hour') or sqlFromNow('-60 days')
function sqlFromNow(mod) {
  const m = mod.match(/^([+-])\s*(\d+)\s*(second|minute|hour|day)s?$/);
  if (!m) return toSQL();
  let ms = parseInt(m[2]) * 1000;
  const unit = m[3];
...
// Client-side time: output ISO datetime for <time> element, JS converts to local
function timeTag(sqlDate, opts) {
  ...
  // Render what the server thinks is right as fallback, JS overrides on load
...
module.exports = { now, toISO, toISOString, today, yesterday, toSQL, sqlFromNow, syncTime, timeTag, offsetMs, synced };
