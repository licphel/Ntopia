...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (!ALLOWED_MIME.includes(file.mimetype)) {
      const err = new Error('仅允许上传图片文件 (JPEG/PNG/WebP/GIF/SVG)');
      err.status = 400;
      return cb(err, false);
    }
    cb(null, true);
...
// Cache sidebar data (TTL 60s) to reduce per-request DB queries
let sidebarCache = null;
let sidebarCacheTime = 0;
const SIDEBAR_CACHE_TTL = 60 * 1000; // 60 seconds
