...
function init(opts) {
  ...
  // ── Drop legacy excerpt column (replaced by firstNLines preview) ─
  for (const table of ['posts', 'post_revisions']) {
    try { db.exec(`ALTER TABLE ${table} DROP COLUMN excerpt`); } catch (_) { /* not present or SQLite too old */ }
  }
