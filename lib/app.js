...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!ALLOWED_MIME.includes(file.mimetype) || !ALLOWED_EXT.includes(ext)) {
      const err = new Error('仅允许上传图片文件 (JPEG/PNG/WebP/GIF)');
      err.status = 400;
      return cb(err, false);
    }
    cb(null, true);
  ...
  if (req.session.user) {
    try {
      ...
      if (now - lastRefresh > 60000 || req.session._needsRefresh) {
        ...
        const u = db.prepare('SELECT id, xp, level, banned, banned_until, display_name, avatar, role, email FROM users WHERE id = ?').get(req.session.user.id);
        if (u) {
...
app.post('/upload', (req, res, next) => {
  ...
  try {
    ...
    // Delete original file — never serve unprocessed uploads
