...
function setCode(email, code) {
  ...
  db.prepare("INSERT INTO email_codes (email, code, expires_at) VALUES (?, ?, datetime('now', '+5 minutes'))").run(email, code);
}

function verifyCode(email, code) {
  const db = getDB();
  // Cleanup expired codes for this email
  db.prepare("DELETE FROM email_codes WHERE email = ? AND expires_at < datetime('now')").run(email);

  const row = db.prepare("SELECT * FROM email_codes WHERE email = ? AND expires_at >= datetime('now') ORDER BY created_at DESC LIMIT 1").get(email);
  if (!row) return false;
...
setInterval(() => {
  try {
    getDB().prepare("DELETE FROM email_codes WHERE expires_at < datetime('now')").run();
  } catch (_) {}
}, 10 * 60 * 1000).unref();
